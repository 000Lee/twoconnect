...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
   try {
      ...
      console.log('친구 요청 수락 시도:', { requestId, userId })
      ...
      console.log('친구 요청 조회 결과:', { connectionData, fetchError })

      if (fetchError || !connectionData) {
         console.error('친구 요청 조회 실패:', { fetchError, connectionData, requestId, userId })
         return NextResponse.json({ success: false, error: '유효하지 않은 친구요청입니다.' }, { status: 400 })
      }

      // 친구요청 상태를 accepted로 변경
      const { error: updateError } = await supabase.from('connections').update({ status: 'accepted' }).eq('id', requestId)

      if (updateError) {
         console.error('친구요청 수락 오류:', updateError)
         return NextResponse.json({ success: false, error: '친구요청 수락 중 오류가 발생했습니다.' }, { status: 500 })
      }
