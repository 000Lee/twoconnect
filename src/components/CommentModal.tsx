         ...
         <ModalContent onClick={(e) => e.stopPropagation()}>
            ...
            {userNickname ? (
               <CommentForm onSubmit={handleSubmitComment}>
                  <CommentInput value={newComment} onChange={(e) => setNewComment(e.target.value)} placeholder="댓글을 입력하세요..." disabled={submitting} required />
                  <SubmitButton type="submit" disabled={submitting || !newComment.trim()}>
                     {submitting ? '작성 중...' : '댓글 작성'}
                  </SubmitButton>
               </CommentForm>
            ) : (
               <div style={{ textAlign: 'center', padding: '20px', color: '#6b7280' }}>댓글을 작성하려면 로그인이 필요합니다.</div>
            )}
                     ...
                     {comments.map((comment) => (
                        <CommentItem key={comment.id}>
                           ...
                           {editingComment?.id === comment.id ? (
                              <EditForm>
                                 <EditTextarea value={editContent} onChange={(e) => setEditContent(e.target.value)} placeholder="댓글을 수정하세요..." />
                                 <EditButtons>
                              ...
                              <>
                                 ...
                                 {userNickname && comment.nickname === userNickname && (
                                    <CommentActions>
                                       <EditButton onClick={() => handleEditComment(comment)}>수정</EditButton>
